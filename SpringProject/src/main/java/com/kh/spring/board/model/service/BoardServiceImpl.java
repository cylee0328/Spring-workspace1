...
public class BoardServiceImpl implements BoardService{
	 ...
	 * service, service내부의 코드 수행 시점에서 에러 발생시 자동 rollback이 된다. 
	 * 
	 * 방법 1) aop를 활용하는 방법 (현재는 안된다)
	 * 
	...
	public int insertBoard(Board b, List<MultipartFile> list, String webPath, String serverFolderPath) throws Exception{
		...
		if(boardNo > 0 && list!=null) {
			...
			// boardImgList : db에 등록한 데이터를 모아놓은 컬렉션 
			...
			for(int i =0; i<list.size(); i++) {
				...
				if(list.get(i).getSize() > 0) { // i번째 요소에 업로드된 이미지가 존재하는지 체크
					...
					// BoardImg객체를 생성해서 필드에 값을 세팅한 후 boardImageList에 추가할예정
		...
		if(result > 0) {
			...
			if(list != null) {
				...
				if(deleteList != null && deleteList.equals("")) {
					...
					result = boardDao.deleteBoardImg(map);
				}
				...
				if(result > 0 ) {
					...
					for(BoardImg img : boardImgList) {
						...
						// 6) 결과값이 0 => update에는 실패했고, 실제로 db에는 올라가야할 데이터이기 때문에 insert 시켜준다.
						if(result == 0) {
